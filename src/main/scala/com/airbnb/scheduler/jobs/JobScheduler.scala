...
class JobScheduler @Inject()(val scheduleHorizon: Period,
			     ...
			     val clusterName: String = "Default-Cluster-Name")
        ...
        case Some(job) => {
          ...
          if (hasAttemptsLeft && (job.lastError.length == 0 || hadRecentSuccess)) {
            ...
            if (disableJob) {
              ...
              message match {
                case Some(message) =>
                  sendNotification(job, "[%s] [Chronos] JOB DISABLED: '%s'".format(clusterName, job.name),
              ...
              message match {
                case Some(message) =>
                  sendNotification(job, "[%s] [Chronos] job '%s' failed!".format(clusterName, job.name),
