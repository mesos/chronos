...
class JobSchedulerSpec extends SpecificationWithJUnit with Mockito {
  ...
  "JobScheduler" should {
    ...
    "Old schedule streams are removed but newer ones are kept" in {
      ...
      val scheduler = new JobScheduler(horizon, mockTaskManager, mockGraph, mockPersistenceStore, jobMetrics = mock[JobMetrics], jobStats = mock[JobStats])
    ...
    "Multiple tasks must be scheduled if they're within epsilon and before time-horizon" in {
      ...
      val scheduler = new JobScheduler(horizon, mockTaskManager, mockGraph, mock[PersistenceStore], jobMetrics = mock[JobMetrics], jobStats = mock[JobStats])
    ...
    "Job scheduler sans streams is empty" in {
      val scheduler = new JobScheduler(Period.hours(1), null, null,
        mock[PersistenceStore], jobMetrics = mock[JobMetrics], jobStats = mock[JobStats])
...
//        }
  ...
  "Missed executions have to be skipped" in {
    ...
    val scheduler = new JobScheduler(epsilon, mockTaskManager, jobGraph, mockPersistenceStore, jobMetrics = mock[JobMetrics], jobStats = mock[JobStats])
